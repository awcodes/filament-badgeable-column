@php
    ...
    $prependedBadges = $getPrependedBadges();
    $appendedBadges = $getAppendedBadges();
    $hasPrependedBadges = collect($prependedBadges)->filter(fn ($badge) => ! $badge->isHidden())->count() > 0;
    $hasAppendedBadges = collect($appendedBadges)->filter(fn ($badge) => ! $badge->isHidden())->count() > 0;
@endphp
...
<div
    {{ $attributes
     ...
     x-data
     x-load-css="['{{ asset('css/badger/badger.css') }}']"
>
                ...
                <{{ $isListWithLineBreaks ? 'li' : 'div' }}>
                    <div @class([
                        ...
                        >
                            @if (filled($formattedState))
                                <div>
                                    @if ($hasPrependedBadges)
                                        @foreach ($prependedBadges as $badge)
                                            {{ $badge }}
                                        @endforeach
                                        <span class="opacity-50">&mdash;</span>
                                    @endif
                                    {{ $formattedState }}
                                    @if ($hasAppendedBadges)
                                        <span class="opacity-50">&mdash;</span>
                                        @foreach ($appendedBadges as $badge)
                                            {{ $badge }}
                                        @endforeach
                                    @endif
                                </div>
